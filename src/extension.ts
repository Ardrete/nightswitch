	...
	return commands.registerCommand("extension.toggleTheme", () => {
		...
		if (currentTheme === nsconfig.get<string>("dayTheme")) {
			...
			window.showInformationMessage(
				"Your current theme is not set to either your day or night theme. Please update your settings."
			);
		}
...
function showAutoSwitchMsg() {
	window
		.showInformationMessage(
			'Automatic switching has been disabled. To reenable, use the command "Enable Automatic Theme Switching".',
			"Click here to reenable"
		)
		.then(fulfilled => {
			autoSwitch = true;
			recheck();
...
function recheck() {
	...
	if (
		...
		window.showWarningMessage(
			'Please set your coordinates in decimal degrees so that NightSwitch-lite can parse them (example: "(49.89,-97.14)").'
		);
		return;
	}
	...
	if (coords != null) {
		...
		const calculatedTimes = SunCalc.getTimes(
			currdate,
			coords[0],
			coords[1]
		);
	...
	if (os.platform() === "darwin" && nsconfig.get("checkDarkMode")) {
		darkMode.isDark().then(isDark => {
			isDark ? setThemeNight() : setThemeDay();
			const status = isDark ? "dark" : "light";
		});
		return;
	}
	...
	if (srisestr != null) {
		...
		if (Number.isNaN(srisetime)) {
			window.showWarningMessage(
				"Something went wrong with your manually set sunrise time. Please use 24-hour format (HH:MM)."
			);
			return;
		}
	}
	...
	if (ssetstr != null) {
		...
		if (Number.isNaN(ssettime)) {
			window.showWarningMessage(
				"Something went wrong with your manually set sunset time. Please use 24-hour format (HH:MM)."
			);
			return;
		}
	}
	...
	if (typeof srisetime == "undefined" || typeof ssettime == "undefined") {
		window
			.showInformationMessage(
				"Please edit your settings to specify how NightSwitch will locate you.",
				"Go to settings"
			)
			.then(fulfilled => {
